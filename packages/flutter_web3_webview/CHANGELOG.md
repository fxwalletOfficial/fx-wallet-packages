...
* CHORE: Regenerate `lib/js/provider.min.js` from the now-vendored `provider/` source tree (esbuild IIFE). The bundle's wire contract (`window.fxwallet`, `FxWalletHandler` bridge, `solana_account` / `solana_signMessage` / `solana_signTransaction` payload shapes, the Solana wallet-standard registration, `Not init finished` guard, `isFxWallet` flag, etc.) is preserved. `EthereumProvider.request` now mirrors the legacy fork's single-line passthrough: the DApp's original `eth_*` / `personal_*` / `wallet_*` method names reach the Dart `Web3RequestDispatcher` untouched (no `MobileAdapter` rename in the middle). Removing the unreachable `MobileAdapter` / `@metamask/eth-sig-util` import chain on both providers also shrinks the artifact from ~1.46 MB to ~320 KB. Behaviour should still be validated against the wallet's real-DApp regression set before tagging a release.
