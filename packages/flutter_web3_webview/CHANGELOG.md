...
* CHORE: Regenerate `lib/js/provider.min.js` from the now-vendored `provider/` source tree (esbuild IIFE, ~2.95 MB vs the legacy ~1.46 MB). The bundle's wire contract (`window.fxwallet`, `FxWalletHandler` bridge, `solana_account` / `solana_signMessage` / `solana_signTransaction` payload shapes, the Solana wallet-standard registration, `Not init finished` guard, `isFxWallet` flag, etc.) is preserved. The size growth is driven by the newer Solana / wallet-standard dependency tree; behaviour should still be validated against the wallet's real-DApp regression set before tagging a release.
