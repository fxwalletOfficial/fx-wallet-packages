...
- Behavior change (malformed input only): `BtcSignature.fromUR` now wraps `ur.decodeCBOR()` so a `crypto-psbt` UR with an undecodable CBOR payload throws `InvalidCborURException` instead of leaking the underlying CBOR decode exception, matching the BCH/XRP `fromUR` factories.
