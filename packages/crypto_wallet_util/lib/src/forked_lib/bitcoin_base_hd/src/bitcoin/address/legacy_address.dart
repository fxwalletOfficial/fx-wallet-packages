...
abstract class LegacyAddress implements BitcoinAddress {
  ...
  LegacyAddress.fromAddress(
    ...
    final decode = decodeLegacyAddress(address, type, network: network);
    if (decode == null) {
  ...
  String toAddress(BasedUtxoNetwork network) {
    ...
    return _legacyToAddress(network, hashToBytes);
  }

  String _legacyToAddress(BasedUtxoNetwork network, List<int> scriptBytes) {
...
class P2pkAddress extends LegacyAddress {
  ...
  String toAddress(BasedUtxoNetwork network) {
    ...
    return _legacyToAddress(network, ripemd160Hash);
  }
