...
class BtcAddressType {
  ...
  static BtcAddressType p2sh = BtcAddressType._('p2sh', 45, '3', 'P2SH', false,
      true, 0x0488b21e, 0x043587cf, getWrongAddress, getP2shAddress);

  /// Address type for P2WSH(Segwit Multisig) address.
  static BtcAddressType p2wsh = BtcAddressType._('p2wsh', 48, 'bc1', 'P2WSH',
      true, true, 0x02aa7ed3, 0x02575483, getWrongAddress, getP2wshAddress);

  /// List of all address types.
  static List<BtcAddressType> get values =>
      [p2pkh, p2wpkh, p2wpkhInP2sh, p2sh, p2wsh];

  /// Get the address type from the script type.(P2PKH, P2WPKH, P2WSH-in-P2SH, P2SH, P2WSH)
  static BtcAddressType getAddressTypeFromScriptType(String addressType) {
    for (BtcAddressType type in values) {
  ...
  /// @nodoc
  static BtcAddressType getAddressTypeByVersion(int version) {
    for (BtcAddressType type in values) {
      if (type.versionForTestnet == version ||
  ...
  static String getP2wpkhAddress(String publicKey) {
    ...
    //int version;
    final program = sha160fromHex(publicKey);
  ...
  static String getP2pkhAddress(String publicKey) {
    ...
    final ripemd160HashOfSha256 = sha160fromHex(publicKey);
    ...
    final sha256HashOfExtendedRipemd160 =
        getSHA256Digest(extendedRipemd160Hash);

    final sha256HashOfSha256Hash =
        getSHA256Digest(sha256HashOfExtendedRipemd160);
  ...
  static String getP2wpkhInP2shAddress(String publicKey) {
    ...
    var scriptSig =
        Uint8List.fromList([...push_20, ...sha160fromHex(publicKey)]);
    var prefix = 0;
    if (isTestnet) {
      prefix = 0xc4;
    } else {
    ...
    var address = (Uint8List.fromList([prefix, ...sha160fromByte(scriptSig)]));
  ...
  static String getP2shAddress(
    ...
    Uint8List redeemScriptHash =
        sha160fromByte(Uint8List.fromList(redeemScript));
    var networkPrefix = isTestnet ? 0xC4 : 0x05;
    var addressBytes = [networkPrefix, ...redeemScriptHash];
    var base58Address = getBase58Address(Uint8List.fromList(addressBytes));
  ...
  static String getP2wshAddress(
    ...
    Uint8List redeemScriptHash =
        getSHA256Digest(Uint8List.fromList(redeemScript));
