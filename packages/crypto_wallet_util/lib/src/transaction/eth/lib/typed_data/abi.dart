...
class AbiUtil {
  ...
  static Uint8List encodeSingle(String type, dynamic arg) {
    ...
    if (isArray(type)) {
      ...
      if (arg is! List) throw ArgumentError('Not an array?');
      ...
      if (arg is String) arg = jsonDecode(arg as String);
      ...
      return setLengthRight(dynamicToUint8List(arg), 32);
      ...
      if (num < BigInt.zero) throw ArgumentError('Supplied uint is negative');

      return encodeBigIntBe(num, length: 32);
      ...
      return encodeBigIntBe(num.toUnsigned(256), length: 32);
    } else if (type.startsWith('ufixed')) {
      size = parseTypeNxM(type);

      var num = parseNumber(arg);
      if (num < BigInt.zero) throw ArgumentError('Supplied ufixed is negative');
  ...
  static Uint8List soliditySHA3(List<String> types, values) {
    return getKeccakDigest(solidityPack(types, values));
  }
  ...
  static Uint8List solidityHexValue(String type, value, bitsize) {
    ...
    if (isArray(type)) {
      ...
      var bytesize = 20;
      if (bitsize != null) bytesize = bitsize ~/ 8;
      ...
      return encodeBigIntBe(num, length: bitsize ~/ 8);
    } else if (type.startsWith('int')) {
      size = parseTypeN(type);
      if ((size % 8 != 0) || (size < 8) || (size > 256)) {
        throw ArgumentError('Invalid int<N> width: $size');
      ...
      bitsize = bitsize != null ? 256 : size;
      return encodeBigIntBe(num.toUnsigned(size), length: bitsize ~/ 8);
    } else {
      throw ArgumentError('Unsupported or invalid type: $type');
    }
  }
  ...
  static BigInt parseNumber(dynamic arg) {
    if (arg is String) {
      if (isHexPrefixed(arg)) {
        return hexToBigInt(dynamicToHex(arg));
      } else {
