...
class Message {
  ...
  factory Message.compile({
    ...
    for (final AddressLookupTableAccount lookupAccount
      ...
      for (int i = 0; i < lookupAddresses.length; ++i) {
        ...
        if (meta != null &&
          ...
          if (i >= 256)
            throw Exception(
                'Address lookup table index $i exceeds the max 255.');
    ...
    writableSigners.sort((a, b) =>
        a.toBase58().toLowerCase().compareTo(b.toBase58().toLowerCase()));
    readonlySigners.sort((a, b) =>
        a.toBase58().toLowerCase().compareTo(b.toBase58().toLowerCase()));
    writableNonSigners.sort((a, b) =>
        a.toBase58().toLowerCase().compareTo(b.toBase58().toLowerCase()));
    readonlyNonSigners.sort((a, b) =>
        a.toBase58().toLowerCase().compareTo(b.toBase58().toLowerCase()));
  ...
  Buffer serialize() {
    ...
    final String blockhash = recentBlockhash;
