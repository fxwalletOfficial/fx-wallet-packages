...
let api_client = AleoAPIClient::<Testnet>testnet();
...
let private_key = PrivateKey::<Testnet>::new(&mut rng).unwrap();
// Encrypt the private key with a password
let private_key_ciphertext = Encryptor::<Testnet>::encrypt_private_key_with_secret(&private_key, "password").unwrap();

// Create the program manager
// (Note: An optional local directory can be provided to manage local program data)
let mut program_manager = ProgramManager::<Testnet>::new(None, Some(private_key_ciphertext), Some(api_client), None, false).unwrap();
...
let record_finder = RecordFinder::<Testnet>::new(AleoAPIClient::testnet());
...
let recipient_key = PrivateKey::<Testnet>::new(&mut rng).unwrap();
