...
mod tests {
    ...
    fn test_constructors_fail_with_multiple_keys_or_no_keys() {
        let api_client = AleoAPIClient::<CurrentNetwork>::testnet3();
        let private_key = PrivateKey::<CurrentNetwork>::from_str(RECIPIENT_PRIVATE_KEY).unwrap();
        let private_key_ciphertext =
            Encryptor::<CurrentNetwork>::encrypt_private_key_with_secret(&private_key, "password")
                .unwrap();
    ...
    fn test_program_management_methods() {
        ...
        let mut program_manager =
            ProgramManager::<CurrentNetwork>::new(Some(private_key), None, None, None, false).unwrap();

        // Test program addition
        let program = Program::<CurrentNetwork>::from_str(HELLO_PROGRAM).unwrap();
        ...
        let program_2 = Program::<CurrentNetwork>::from_str(HELLO_PROGRAM_2).unwrap();
    ...
    fn test_import_resolution() {
        ...
        let vm_execute =
            ProgramManager::<CurrentNetwork>::initialize_vm(&api_client, &top_level_program, true)
                .unwrap();
        let vm_deploy =
            ProgramManager::<CurrentNetwork>::initialize_vm(&api_client, &top_level_program, false)
                .unwrap();
