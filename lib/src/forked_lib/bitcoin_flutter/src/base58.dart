...
/// In order to comply with Bitcoin and Ripple standard encoding Base58Check, use [Base58CheckCodec].
...
class Base58Decoder extends Converter<String, List<int>> {
  ...
  List<int> convert(String input) {
    ...
    for (int i = 0; i < input.length; i++) {
      ...
      if (charInt < 0) throw FormatException('Invalid input formatting for Base58 decoding.');
...
class Base58CheckPayload {
  ...
  @override
  bool operator ==(Object other) => other is Base58CheckPayload && version == other.version && const ListEquality().equals(payload, other.payload);
  @override
  int get hashCode => version.hashCode ^ const ListEquality().hash(payload);
}

/// A codec for Base58Check, a binary-to-string encoding use in cryptocurrencies like Bitcoin and Ripple.
///
/// The constructor requires the alphabet and a function that performs a SINGLE-round SHA-256 digest on a [List<int>] and
...
class Base58CheckCodec extends Codec<Base58CheckPayload, String> {
  ...
  Base58CheckCodec(this.alphabet) : _encoder = Base58CheckEncoder(alphabet), _decoder = Base58CheckDecoder(alphabet);
...
class Base58CheckDecoder extends Converter<String, Base58CheckPayload> {
  ...
  Base58CheckPayload _convert(String encoded, bool validateChecksum) {
    ...
    if (bytes.length < 5) throw FormatException('Invalid Base58Check encoded string: must be at least size 5');

    List<int> checksum = _hash(bytes.sublist(0, bytes.length - 4));
    List<int> providedChecksum = bytes.sublist(bytes.length - 4, bytes.length);
    if (validateChecksum && !ListEquality().equals(providedChecksum, checksum.sublist(0, 4))) throw FormatException('Invalid checksum in Base58Check encoding.');
